  ...
  return (
    <div className="p-12 max-w-7xl mx-auto space-y-12">
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end border-b-4 border-black/10 pb-10 gap-8">
        <div className="text-left">
          ...
          <div className="flex items-center gap-4">
            ...
            <p className="text-black/40 font-black uppercase tracking-[0.4em] text-[10px]">Project Archive Master</p>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
        <button onClick={() => setIsModalOpen(true)} className="h-[300px] border-4 border-dashed border-black/10 texture-hatch flex flex-col items-center justify-center gap-4 text-black/30 font-black uppercase text-[10px] tracking-[0.4em] hover:border-black hover:text-black transition-all group sketch-border bg-white/10">
          <div className="p-4 bg-white border-2 border-black/5 sketch-border group-hover:scale-110 transition-transform"><Plus size={32} className="text-black" /></div>
        ...
        {projects.map((project) => (
          <div key={project.id} className="sketch-card flex flex-col group transition-all overflow-hidden h-[300px] relative">
            <div className="h-32 bg-[#EAE3D9] flex items-center justify-center overflow-hidden texture-hatch-v relative border-b border-black/10">
               ...
               <div className="absolute top-3 left-3 px-2 py-1 bg-black text-white text-[7px] font-black uppercase tracking-widest sketch-border">SEQUENCE ARCHIVE</div>
            </div>
            <div className="p-6 flex flex-col flex-grow bg-white/40 texture-dots">
              <div className="flex-grow">
                <h3 className="font-black text-black text-2xl uppercase italic leading-tight group-hover:text-black transition-all truncate">{project.title}</h3>
                <div className="flex items-center gap-2 mt-2">
                  <div className="h-0.5 w-4 bg-black/20"></div>
                  <p className="text-[9px] font-black text-black/30 uppercase tracking-widest">{project.shots.length} Finalized Shots</p>
                </div>
              </div>
              <div className="mt-auto flex gap-3 pt-4 border-t border-black/5">
                <button onClick={() => onSelectProject(project.id)} className="flex-grow pencil-button py-3.5 font-black text-[10px] uppercase tracking-[0.3em] flex items-center justify-center gap-2 active:scale-95">Enter Studio <ArrowRight size={14} /></button>
                <button onClick={() => onDeleteProject(project.id)} className="p-3.5 border-2 border-black/10 text-black/20 hover:text-red-600 hover:border-red-600/30 transition-all active:scale-95" title="Archive project"><Trash2 size={16} /></button>
              </div>
            </div>
          </div>
        ))}
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[100] bg-[#EAE3D9]/95 backdrop-blur-xl flex items-center justify-center p-6 animate-in fade-in duration-300">
           <div className="sketch-card w-full max-w-lg p-12 relative animate-in zoom-in-95 texture-dots">
             <button onClick={() => setIsModalOpen(false)} className="absolute top-8 right-8 text-black/20 hover:text-black transition-colors"><X size={28} /></button>
             <div className="mb-10">
             ...
             <form onSubmit={(e) => { e.preventDefault(); onCreateProject(newTitle); setNewTitle(''); setIsModalOpen(false); }} className="space-y-10">
               ...
               <button type="submit" disabled={!newTitle.trim()} className="w-full pencil-button py-6 font-black text-xl uppercase tracking-[0.5em] disabled:opacity-20 active:translate-y-1 transition-all">ACTION!</button>
             </form>
           </div>
        </div>
      )}
